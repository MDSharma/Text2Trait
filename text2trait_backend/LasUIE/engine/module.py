...
class StrcutT5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def struct_modeling(self, input_ids, hidden_rep, mask):
        ...
        #  New implementation
        for idx in range(bsz):
            _, stru_s, stru_e = self.build_con_struct(
                distance[idx, :length_valid[idx]], 
                dep_[idx, :length_valid[idx]],
                [], []
            )
            ...
            # --- Structural graph ---
            if is_mps:
                # Keep graph and features on CPU
                stru_s_cpu = torch.tensor(stru_s, dtype=torch.int, device='cpu')
                stru_e_cpu = torch.tensor(stru_e, dtype=torch.int, device='cpu')
                con_f = dgl.graph((stru_s_cpu, stru_e_cpu))
                con_f.ndata['feature'] = hidden_rep[idx, 0:con_f.num_nodes(), :].cpu()
                # Compute representation then move to MPS
                con_rep = self.ConGAT(con_f, con_f.ndata['feature']).to(self.device)
            else:
                stru_s = torch.tensor(stru_s, dtype=torch.int, device=self.device)
                stru_e = torch.tensor(stru_e, dtype=torch.int, device=self.device)
                con_f = dgl.graph((stru_s, stru_e)).to(self.device)
                con_f.ndata['feature'] = hidden_rep[idx, 0:con_f.num_nodes(), :]
                con_rep = self.ConGAT(con_f, con_f.ndata['feature'])

            # --- Dependency graph ---
            dep_src = dep_[idx, :length_valid[idx]]
            dep_dst = head_[idx, :length_valid[idx]]

            if is_mps:
                dep_f = dgl.graph((dep_src.cpu(), dep_dst.cpu()))
                dep_f.ndata['feature'] = hidden_rep[idx, 0:dep_f.num_nodes(), :].cpu()
                dep_rep = self.DepGAT(dep_f, dep_f.ndata['feature']).to(self.device)
            else:
                dep_f = dgl.graph((dep_src, dep_dst)).to(self.device)
                dep_f.ndata['feature'] = hidden_rep[idx, 0:dep_f.num_nodes(), :]
                dep_rep = self.DepGAT(dep_f, dep_f.ndata['feature'])

            # --- Combine and append ---
            total_graph_node = con_rep + dep_rep
            past_key_list.append(total_graph_node)
