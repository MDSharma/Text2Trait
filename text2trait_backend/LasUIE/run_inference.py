...
def on_validation_epoch_end(self):
    ...
    if self.is_train:
        ...
        if self.task_type == TaskType.SPAN:
            ...
            self.log('triplet-F1', avg_triplet_F1)

    self.val_outputs.clear()
    if torch.backends.mps.is_available():
        torch.mps.empty_cache()
        mylogger.info("Cleared MPS GPU cache after validation epoch.")
...
def init_args():
    ...
    parser.add_argument("--model_checkpoint", default=r'checkpoint/pair/re/gwas/finetuned|uie|epoch=0059|step=0000180.ckpt', type=str) # example: checkpoint/pair/re/nyt/finetuned|uie|epoch=0001|step=0000626.ckpt
    ...
    # Force cpu - can change if u have gpu
...
if __name__ == '__main__':
    ...
    for batch in model_output:
        ...
        for sentence, label in zip(batch['sentences'], batch['labels']):
            if label.strip():
                filtered_batch['sentences'].append(sentence)
                filtered_batch['labels'].append(label)
        if filtered_batch['labels']:
            filtered_model_output.append(filtered_batch)

    uie_results = decoding_labels(filtered_model_output, args.task_type)
