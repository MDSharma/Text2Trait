...
class ModelWrapper(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.is_train:
            ...
            output = {'loss': loss.cpu().numpy(), 'rouge': result["rouge"]}
        else:
            result = measuring([Evaluation.ROUGE, Evaluation.UIE], preds, golds, self.task_type)
            if self.task_type == TaskType.SPAN:
                output = {'rouge': result["rouge"], 'span-F1': result["span_F1"]}
            else:
                output = {'rouge': result["rouge"], 'span-F1': result["span_F1"], 'triplet-F1': result["triplet_F1"]}

        self.validation_outputs.append(output)
        return output

    # Chnaged from the orginal
    def on_validation_epoch_end(self):
        ...
        if self.is_train:
            ...
            avg_rouge_score = np.mean(np.array([x['rouge'] for x in outputs]))
...
def init_args():
    ...
    parser.add_argument("--do_eval", action='store_true', default=True)  # True
    ...
    parser.add_argument("--num_train_epochs", default=1000, type=int) # Set whatever you want!
    parser.add_argument("--train_patience", default=250, type=int)
    ...
    # Added to make it avaiable for different cores
...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        train_params = dict(
            ...
            devices=args.n_gpu,
            accelerator="cpu", # when there is a wheel to cuda 12.9 you can switch it to "gpu"
            strategy="auto",
