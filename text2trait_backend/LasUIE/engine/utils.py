...
#Changed from the original for PAIR task
def extract_uie_labels(content_list, task_type):
    ...
    if task_type == TaskType.SPAN:
        ...
        for trp in content_list:
            ...
            sub_structs = trp.get('sub-struct', [])
            if not sub_structs:
                continue  # skip if no sub-structures
            sub = sub_structs[0]  # take first sub-structure
            rel = sub.get('sub-rel', '')
            span_e = sub.get('sub-span', '')
            span_e_attr = sub.get('sub-attr', '')
...
def get_uie_list(main_tree):
    ...
    for start_tree in main_tree:
        ...
        if len(sub_triplets) == 0:
            # Use empty strings instead of None
